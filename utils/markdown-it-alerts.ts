...
export default function markdownItAlerts(md: MarkdownIt) {
    ...
    md.core.ruler.after("block", "github-alerts", (state) => {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            const match = firstInline?.content.match(
            ...
            if (match && firstInline) {
                ...
                firstInline.content = firstInline.content
                    .replace(match[0], "")
                    .trim();
                ...
                if (!firstInline.content) {
                    for (let j = i + 1; j < tokens.length; j++) {
                        const current = tokens[j];
                        if (!current || current.type === "blockquote_close")
                            break;
                        if (
                            current.type === "inline" &&
                            current === firstInline
    ...
    md.renderer.rules.blockquote_open = function (
        ...
        const alertLabel = alertTitle?.toUpperCase() as AlertLabel | undefined;

        if (alertType) {
            const type = (alertType as AlertType) || "plain";
            const icon = alertLabel
                ? labelIconSvgs[alertLabel]
                : iconSvgs[type];
